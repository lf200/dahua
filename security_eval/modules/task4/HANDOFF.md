...
This package owns application-security evaluation for task 4. It only communicates with the rest of the application through the contract models in `security_eval.contracts`.
...
Real runs require an OpenAI-compatible endpoint with native chat tool-calling support. `TARGET_BASE_URL` and `TARGET_API_KEY` are taken from the shared target client, while `APPLICATION_SECURITY_MODEL` selects the API model. The private application-security dependency pinned in `requirements/task4.in` must recognize the model family used by its attack templates. Common model aliases are mapped to adapter-validated snapshots; exact compatible snapshot names are accepted directly. Unknown custom model IDs are rejected by `validate()` before any case starts.

## Benchmark and modes

- Project benchmark version: `v1`
- Private dependency versions: pinned in `requirements/task4.in`
- Adapter-validated suite compatibility: `workspace` version `v1.2.2`
...
Run `pytest tests/task4 -q`. Tests use a fake adapter and static trace data; they never call private application-security dependencies, a model, or the network. The web-ready fixture is `tests/contract_fixtures/task_4_result.json`.
