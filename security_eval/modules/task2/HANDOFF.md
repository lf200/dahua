...
- `validate()` loads `manifest.yaml` through the task-local verified benchmark
  ...
  dynamic test engine availability.
...
- `dynamic` asks the task-local dynamic test engine adapter for up to one variant per
  category in quick and up to three per category in full. dynamic test engine may filter
...
dynamic test engine is imported lazily inside `dynamic_test_adapter.py`. Benchmark-only runs
therefore work without dynamic test engine. A missing dynamic dependency becomes a
sanitized `DEPENDENCY_ERROR`; hybrid retains the completed benchmark results.
No dynamic test engine object crosses the task 2 package boundary.
...
Offline tests mock the target, judge, and dynamic test engine. They cover the 90-case
