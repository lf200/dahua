...
The task dependency file freezes the private dynamic-testing dependency and the transitive packages required by the adapter. Treat that file as the source of truth for compatible versions; callers do not depend on the private package surface.

Load `security_eval/modules/task1/module.json` through `ModuleRegistry`, or include that path in `MODULE_MANIFEST_PATHS`. The manifest creates `security_eval.modules.task1.module.Task1Module` without importing the private dynamic-testing dependency; the dependency is loaded only when `dynamic` or a triggered `hybrid` run needs it.
...
Each dynamic observation carries its own category-level expected behavior, forbidden outcomes, recognition/blocking criteria, and self-contained recovery contract. Dynamic judging uses those fields together with the generated conversation and adapter evidence; it never borrows a concrete scenario or rubric from a static benchmark case.

## Safety and failures

Every target/judge input, response, evidence item, and normalized error passes through `RunContext.sanitize_value()` before the `TaskResult` is returned. A target, judge, parser, deadline, or dynamic-adapter failure creates an `invalid` case and does not stop later cases. Unit tests never call a real model. Before importing private dependencies, the adapter disables their telemetry and result upload.
