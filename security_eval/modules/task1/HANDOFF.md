...
The task dependency file also freezes the adapter-tested evaluation dependency release and two required transitive packages. dynamic test engine 1.0.7 otherwise resolves a broken `pydantic-settings` artifact in the current index and imports `sentry_sdk` without declaring it.

Load `security_eval/modules/task1/module.json` through `ModuleRegistry`, or include that path in `MODULE_MANIFEST_PATHS`. The manifest creates `security_eval.modules.task1.module.Task1Module` without importing dynamic test engine; dynamic test engine is loaded only when `dynamic` or a triggered `hybrid` run needs it.
...
Each dynamic observation carries its own category-level expected behavior, forbidden outcomes, recognition/blocking criteria, and self-contained recovery contract. Dynamic judging uses those fields together with the generated conversation and dynamic test engine evidence; it never borrows a concrete scenario or rubric from a static benchmark case.

## Safety and failures

Every target/judge input, response, evidence item, and normalized error passes through `RunContext.sanitize_value()` before the `TaskResult` is returned. A target, judge, parser, deadline, or dynamic test engine failure creates an `invalid` case and does not stop later cases. Unit tests never call a real model. Before importing either third-party package, the adapter opts out of both dynamic test engine and evaluation dependency telemetry, and it also disables Confident AI result upload.
